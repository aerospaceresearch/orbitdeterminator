...
def kep_state(kep):
	...
	eci_r = np.array([
					  [r[0, 0] * (cos(argper)*cos(raan) - sin(argper)*cos(inc)*sin(raan)) -
					   r[1, 0] * (sin(argper)*cos(raan) + cos(argper)*cos(inc)*sin(raan))],

					  [r[0, 0] * (cos(argper)*sin(raan) + sin(argper)*cos(inc)*cos(raan)) +
					   r[1, 0] * (cos(argper)*cos(inc)*cos(raan) - sin(argper)*cos(raan))],

					  [r[0, 0] * (sin(argper)*sin(inc) + r[1, 0] * (cos(argper)*sin(inc)))],

					  [r[3, 0] * (cos(argper) * cos(raan) - sin(argper) * cos(inc) * sin(raan)) -
					   r[4, 0] * (sin(argper) * cos(raan) + cos(argper) * cos(inc) * sin(raan))],

					  [r[3, 0] * (cos(argper) * sin(raan) + sin(argper) * cos(inc) * cos(raan)) +
					   r[4, 0] * (cos(argper) * cos(inc) * cos(raan) - sin(argper) * cos(raan))],

					  [r[3, 0] * (sin(argper) * sin(inc) + r[1, 0] * (cos(argper) * sin(inc)))]
					  ])
