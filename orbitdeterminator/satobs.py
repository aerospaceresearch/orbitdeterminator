...
def print_iod(iod):
    ...
    Returns:
        a_t, a_azimuthDeg,  a_elevationDeg,  a_slantRange, obs_lat, obs_long, obs_alt,len(iod)
    '''

    obs_lat=input('Enter observatory latitude:')
    obs_long=input('Enter observatory longitude:')
    obs_alt=input('Enter observatory altitude(m):')
    range_avail=input('Is slant range available?[y/n]\n')
    if(range_avail=='n'):
         print('Code is not ready yet.Exiting...')
         sys.exit()    
    ...
    for i in range(0, len(iod)):
        ...
        a_azimuthDeg.append(re.split(" ", position_deg_str)[0])                   
        a_elevationDeg.append(re.split(" ", position_deg_str)[2 if len(re.split(" ", position_deg_str)) > 2 else 1])
        a1_t=(datetime_to_str(iod_data[4]))
        ...
        #stripping off the error part                                                    #check:figure out range part for iod
        ''' 
            r1=range_to_str_uk(iod_data[6])
            #stripping off the error part
            a_slantRange
            this_r=''
            for x in range(0,10):
        ...
        if(ord(iod[i][33]) > 51):                                                        #for ra/dec to az/el
               pass
        else:
               a_azimuthDeg[i],a_elevationDeg[i]=pm.radec2azel(float(a_azimuthDeg[i]),float(a_elevationDeg[i]), obs_lat, obs_long,a_t[i], usevallado=False)   #time in whic format?

   #for i in range(0,len(iod)):
   #    print(float(a_azimuthDeg[i]), float(a_elevationDeg[i]), float(a_slantRange[i]), float(obs_lat), float(obs_long), float(obs_alt))

    return a_t,a_azimuthDeg,a_elevationDeg,a_slantRange, obs_lat, obs_long, obs_alt               
...
def print_uk(uk):
    ...
    Returns:
        a_t, a_azimuthDeg,  a_elevationDeg,  a_slantRange, obs_lat, obs_long, obs_alt(metre),len(uk)
    '''
    ...
    for i in range(0, len(uk)):
        ...
        a1_t=(datetime_to_str(uk_data[2]))
        temp=''
        for x in range(0,40):
            if(a1_t[x]=='±'):
                break
        ...
        r1=range_to_str_uk(uk_data[6])                                                  #stripping off the error part
        a_slantRange
        this_r=''
        for x in range(0,10):
        ...
        a_slantRange.append(this_r)                                                     #for ra/dec to az/el
        if (ord(uk[i][33]) > 51):
            pass
        else:
            a_azimuthDeg[i],a_elevationDeg[i]=(pm.radec2azel(float(a_azimuthDeg[i]),float(a_elevationDeg[i]),float(obs_lat),float(obs_long),pm.str2dt(a_t[i])))   
                                                                                                        

    #for i in range(0,len(uk)):
    #    print(float(a_azimuthDeg[i]), float(a_elevationDeg[i]), float(a_slantRange[i]), float(obs_lat), float(obs_long), float(obs_alt),pm.str2dt(a_t[i]))

    return a_t,a_azimuthDeg,a_elevationDeg,a_slantRange, obs_lat, obs_long, obs_alt,len(uk)               
...
'''def aer_2_ecef(azimuthDeg, elevationDeg, slantRange, obs_lat, obs_long, obs_alt):

    #site ecef in kilometers
    sitex, sitey, sitez = pm.geodetic2ecef(obs_lat,obs_long,obs_alt)
    
    #some needed calculations
    slat = math.sin(math.radians(obs_lat))
    slon = math.sin(math.radians(obs_long))
    clat = math.cos(math.radians(obs_lat))
    clon = math.cos(math.radians(obs_long))
...
if(__name__ == "__main__"):
    ...
    file_type = input("Select file format:\n 1.IOD.\n 2.UK.\n 3.RDE.\n")
    ...
    if(file_type=='1'):
      ...
      # iod_file.append("23794 96 010A   2701 P 20040506061735610 17 25 1909776-205541 97 I")
        ...
        a_t,a_azimuthDeg,a_elevationDeg,a_slantRange, obs_lat, obs_long, obs_alt,len_iod= print_uk(iod_file)

        print("<------Printing corresponding ecef coordinates in [t,x,y,z] format------>")
        print(" ")
        a_unix_t=[]

        for i in range(0, len_iod):
            x,y,z=pm.aer2ecef(float(a_azimuthDeg[i]), float(a_elevationDeg[i]), float(a_slantRange[i]), float(obs_lat), float(obs_long), float(obs_alt))
            a_unix_t.append(dateutil.parser.parse(a_t[i],dayfirst=True).timestamp())
            print(a_unix_t[i],x,y,z)
        print(" ")
      ...
      # uk_file.append("9107605267504050320483131  010  12152189  +32549   20 5")
        ...
        a_t,a_azimuthDeg,a_elevationDeg,a_slantRange, obs_lat, obs_long, obs_alt,len_uk= print_uk(uk_file)  
       
        print("<------Printing corresponding ecef coordinates in [t,x,y,z] format------>")
        print(" ")
        a_unix_t=[]

        for i in range(0, len_uk):
            x,y,z=pm.aer2ecef(float(a_azimuthDeg[i]), float(a_elevationDeg[i]), float(a_slantRange[i]), float(obs_lat), float(obs_long), float(obs_alt))
            a_unix_t.append(dateutil.parser.parse(a_t[i],dayfirst=True).timestamp())
            print(a_unix_t[i],x,y,z)
        print(" ")
